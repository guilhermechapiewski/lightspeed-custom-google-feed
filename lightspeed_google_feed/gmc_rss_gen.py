...
class GMCRSSGenerator:
    ...
    def refresh_feed_files(self, cloud=False):
        ...
        products_for_template = self.prepare_template_data(products)
            ...
            for product in products:
                ...
                for product_variant in product["variants"].values():
                    try:
                        ...
                        self.logger.debug(f"Product categories for product id {product_id}: {product_categories} (type: {type(product_categories)})")
                        ...
                        self.template_data.append(template_data)
